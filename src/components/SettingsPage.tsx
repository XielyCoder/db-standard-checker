...
import { Bot, Sparkles, Code, FileText, Globe, Lightbulb, Database, Settings as SettingsIcon, FileSearch, Shield, Search, BarChart3 } from 'lucide-react';
...
const PRESET_COLORS = [
  '#0d9488', '#0052d9', '#0594fa', '#00a870', '#ed7b2f', 
  '#e34d59', '#7c3aed', '#ea580c', '#5c6bc0', '#26a69a'
];
...
const PRESET_TEMPLATES = [
  {
    name: '安全合规检测',
    description: '检查数据库安全合规性，包括敏感字段加密、权限设计',
    systemPrompt: '你是一个数据库安全合规检测助手。你擅长检查数据库表结构中的安全合规问题，包括敏感字段加密存储、权限设计、数据脱敏等方面。请帮助用户发现并修复安全隐患。',
    icon: 'Shield',
    color: '#e34d59',
  },
  {
    name: '性能优化分析',
    description: '分析表结构性能问题，优化索引和查询设计',
    systemPrompt: '你是一个数据库性能优化分析助手。你擅长分析数据库表结构的性能问题，包括索引设计优化、查询性能评估、分区策略建议等。请帮助用户提升数据库查询性能。',
    icon: 'BarChart3',
    color: '#0594fa',
  },
  {
    name: '迁移方案生成',
    description: '生成数据库表结构迁移方案和 DDL 脚本',
    systemPrompt: '你是一个数据库迁移方案生成助手。你擅长根据贯标检测结果生成表结构迁移方案，包括 ALTER TABLE 语句、数据迁移脚本、回滚方案等。请确保迁移方案的兼容性和安全性。',
    icon: 'Code',
    color: '#7c3aed',
  },
];
...
export function SettingsPage({ 
  ...
  const handleEdit = (agent: CustomAgent) => {
    const builtinIds = ['default', 'standard-config', 'ddl-parser'];
    if (builtinIds.includes(agent.id)) return;
  ...
  const builtinIds = ['default', 'standard-config', 'ddl-parser'];
  const customAgents = agents.filter(a => !builtinIds.includes(a.id));
